...
Manipulation::Manipulation() : Node("manipulation") {
  ...
  pick_client = create_client<gazebo_msgs::srv::DeleteEntity>("delete_entity");
  place_client = create_client<gazebo_msgs::srv::SpawnEntity>("spawn_entity");
  manpltn_node = rclcpp::Node::make_shared("book_manipulation_node");
}
...
bool Manipulation::pick_book() {
  ...
  auto ret = rclcpp::spin_until_future_complete(manpltn_node, result, 10s);
...
bool Manipulation::place_book() {
  auto res = system(
      "ros2 run gazebo_ros spawn_entity.py -entity book -x 3.5 -y -2.5 -z 0 "
      "-file `ros2 pkg prefix "
      "808x-final-project`/share/808x-final-project/models/book/model.sdf");
  if (res > 0) {
    return true;
  } else {
    return false;
