...
void Navigation::odom_callback_search(
  ...
  if ((std::abs(static_cast<int>(msg->pose.pose.position.y - req_pos_y)) ==
       0) &&
      (std::abs(static_cast<int>(msg->pose.pose.position.x)) == 0)) {
    check_odom = true;
  }
}
...
bool Navigation::search_book() {
  ...
  auto odom_sub = nav_odom_node->create_subscription<nav_msgs::msg::Odometry>(
      "odom", 10,
      std::bind(&Navigation::odom_callback_search, this,
                std::placeholders::_1));
  // Start the searching
  while (search_pos.size() > 0) {
    float_t pop_pos = search_pos.back();
    search_pos.pop_back();
    RCLCPP_INFO(this->get_logger(), "In Search Bins %ld %f", search_pos.size(),
                pop_pos);
    check_odom = false;
    req_pos_y = pop_pos;
